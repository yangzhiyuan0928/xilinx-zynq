...
static int axi_adc_dma_init(struct axi_dma_adc_dev *adc_dev)
{
   ...
   adc_dev->rx_chan = dma_request_slave_channel(&adc_dev->pdev->dev, adc_dev->chan_name);
    if (!adc_dev->rx_chan) {
...
//    minor = (int) filep->private_data;
    ...
    dma_sync_single_for_cpu(adc_dev->rx_chan->device->dev,
		            adc_dev->dma_dsts+ adc_dev->package_no*AXI_ADC_BUFF_SIZE,
                            AXI_ADC_BUFF_SIZE, DMA_DEV_TO_MEM);    
    status = copy_to_user(buf, 
                          adc_dev->dsts+ adc_dev->package_no*AXI_ADC_BUFF_SIZE,
                          AXI_ADC_BUFF_SIZE);
    adc_dev->package_no++;
    if (adc_dev->package_no >= AXI_ADC_BUFF_NUM )
        adc_dev->package_no = 0;
...
static const struct of_device_id axidma_adc_of_ids[] = {
    {.compatible = "xlnx,ad-fifo-m-s2mm-1.0",},
    {} 
};
...
static int axidma_adc_probe(struct platform_device *pdev)
{
  ...
  /* Allocate a private structure to manage this device */
   ...
   printk(KERN_EMERG "chan_name = %s\n",axidma_adc_dev->chan_name);
   if (status){
