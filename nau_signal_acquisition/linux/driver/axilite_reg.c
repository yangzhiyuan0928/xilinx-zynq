...
int axilite_reg_fasync(int fd,struct file *filep,int mode)  
{
    ...
    retval=fasync_helper(fd,filep,mode,&xdev->async_queue);  
...
static irqreturn_t axilite_reg_interrupt_handler(int irq, void *devid)
{
  ...
  if(xdev->async_queue)  
  {
       	kill_fasync(&xdev->async_queue,SIGIO,POLL_IN);  
  }
...
static int axilite_reg_probe(struct platform_device *pdev)
{
	...
	AXILITE_REG_MINOR_START, AXILITE_REG_MINOR_COUNT,MODULE_NAME);  
	if (status < 0) {
	...
	cdev_init(&xdev->cdev, &axilite_reg_fileops);  
	...
	devp = device_create( axilite_reg_class,NULL,xdev->devno,NULL,MODULE_NAME);	
	if(IS_ERR(devp))	
...
{      	
	...
	unregister_chrdev_region(xdev->devno, AXILITE_REG_MINOR_COUNT); 
...
static struct platform_driver axilite_reg_driver = {     
	...
	.driver = {
		.name = MODULE_NAME,     
		.owner = THIS_MODULE,
		.of_match_table = axilite_reg_of_match_table,  
	},
};
